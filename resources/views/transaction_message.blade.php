@extends('layouts.app', ['page_title' => 'Transaction terminée'])
            ...
            <div class="container">
                <div class="row justify-content-center align-items-center">
                    <div class="col-lg-9 text-center mt-5">
                        <h1 class="heading" data-aos="fade-up">Transaction terminée</h1>

                        <nav aria-label="breadcrumb" data-aos="fade-up" data-aos-delay="200">
                            <ol class="breadcrumb text-center justify-content-center">
                                <li class="breadcrumb-item"><a href="{{ route('home') }}">Accueil</a></li>
                                <li class="breadcrumb-item active text-white-50" aria-current="page">Transaction d'argent depuis la plateforme AddrressImmo</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
@if (Route::is('transaction.waiting'))
                        <div style="margin: 3rem 0;">
                            ...
                            <p class="lead">Veuillez valider le message de votre opérateur sur votre téléphone. Ensuite appuyez sur le bouton ci-dessous.</p>
                            <p>
                                <a href="{{ route('transaction.message', ['app_id' => '', 'orderNumber' => explode('-', request()->success_message)[0], 'userId' => explode('-', request()->success_message)[1]]) }}" class="btn adrm-btn-green py-3 px-5 rounded-pill">
...
@else
    @if (!empty($status_code))
                        ...
                        <div style="margin: 3rem 0;">
        @if ($status_code == '0')
                            <h1 class="text-success" style="font-size: 10rem; margin-bottom: 3rem;"><span class="bi bi-check-circle text-success"></span></h1>
        @endif

        @if ($status_code == '1')
                            <h1 class="text-warning" style="font-size: 10rem; margin-bottom: 3rem;"><span class="bi bi-exclamation-circle text-warning"></span></h1>
        @endif

        @if ($status_code == '2')
                            <h1 class="text-danger" style="font-size: 10rem; margin-bottom: 3rem;"><span class="bi bi-x-circle text-danger"></span></h1>
        @endif
    @endif
    ...
    @if (!empty($payment))
                        <div class="clearfix" style="background-color: #e0e0e0; margin-bottom: 5rem; padding: 10px;">
                            <div class="pull-left text-left">
                                ...
                                <p style="margin: 0"><small>{{ ucfirst(explicitDate($payment->created_at)) }}</small></p>
                            </div>
                            <div class="pull-right" style="padding-top: 1.5rem;">
                                ...
                                <span class="badge rounded-pill" style="font-weight: 600; background-color: {{ $payment->status == 0 ? 'green' : ($payment->status == 1 ? 'orange' : 'red') }}">
                                    {{ $payment->status == 0 ? 'Transaction réussie' : ($payment->status == 1 ? 'Transaction en attente' : 'Transaction échouée') }}
                                </span>
                            </div>
                        </div>
    @endif
